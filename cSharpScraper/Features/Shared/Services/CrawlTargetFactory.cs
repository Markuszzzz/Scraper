...
{
    public static async Task<CrawlTarget?> CreateAsync(string target, bool crawlSubdomains, ILogger<CrawlTarget> logger)
    {
        return crawlSubdomains
            ? await FromRegistrableDomainAsync(target, logger)
            : await FromFullUrlAsync(target, logger);
    }
